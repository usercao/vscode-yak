# vscode-yak VS Code 扩展竞品对比与语法决策
...
>
> 本文比较的是当前仓库的实际实现，而非设计文档中的远期设想。竞品源码也以该日期拉取的默认分支为准；它们并不是 yak 的官方编辑器实现。

## 结论

当前扩展已完成 P1 的静态 CSS 语言体验：在 TS、TSX、JS、JSX 的 yak tagged template 中提供 CSS 高亮、补全、hover、诊断、快速修复和颜色 picker，并把 `${...}` 内交还给 JavaScript/TypeScript。语义识别、虚拟 CSS 映射和真实 Extension Host 测试基础设施已经实现。
...
- `yak` 官方定位是“使用 styled-components 风格 API 的标准 CSS 语法”，并明确展示原生 CSS 嵌套、媒体查询和 keyframes。
...
- 这说明其编辑器实现借用 SCSS 解析能力来容忍 CSS-in-JS 常见的嵌套和插值，**不等于** styled-components 或 yak 的运行时会编译完整 Sass。
- 两个参照仓库都没有接入 `getLESSLanguageService` 或 `source.css.less`，没有证据表明它们支持 Less。
- 在 yak 的编译器明确支持前，本扩展应保持 CSS 为默认且唯一承诺的语言模式；不应把 `$var`、`@mixin`、`@include`、`@use` 或 Less 的 `@var`、mixin 当成受支持语法高亮或补全。

因此，当前项目的“CSS 模式”是符合 yak 官方契约的选择；不是因为 CSS-in-JS 天生排斥 SCSS，而是因为编辑器不能承诺上游编译器不支持的语法。

## 对比对象的角色

| 项目 | 角色 | 与本项目的关系 |
| --- | --- | --- |
| 当前 `vscode-yak` | 独立 VS Code 扩展 | 面向 yak；当前将高亮和 CSS 补全放在扩展宿主内实现。 |
...
- 使用 TypeScript AST 确认直接、别名和命名空间 `yak` import；识别 `styled.tag`、`styled(Component)`、类型参数、`.attrs(...)`、`css`、`globalStyle`、`keyframes`，并排除 type-only import、局部遮蔽和动态 tag path。
...
- 通过 `DiagnosticCollection` 暴露安全映射的 CSS 诊断，并以 `yak.css.validate` 按资源控制；代码操作目前只接受同文档、非重叠、单行的安全 CSS 编辑。
...
| --- | --- | --- | --- |
| CSS 高亮 | 注入标准 `source.css` grammar，覆盖显式泛型、`.attrs(...)`、静态 element access、namespace、CSS prop 和 keyframes；以低影响 tag decoration 标记静态 pattern match。 | 有更成熟的 styled-components grammar，覆盖许多历史 template 形式，并使用 `source.css.scss`。 | 保持 yak 专用、CSS-only 的静态结构识别；新增上游 API 形态时按四种宿主语言补 scope 与宿主恢复回归，不猜测 alias。 |
| 模板识别 | TypeScript AST 验证 `yak` import，支持别名、命名空间、常见 styled 链、嵌套模板和静态 element access，并排除 type-only import 与局部遮蔽。 | TypeScript plugin 也以可配置 tag 名为中心，但可通过 tsserver 装饰器接入语言服务。 | 后续扩展跨文件 re-export/barrel import、用户确认的额外 tag 与 import module；不要让 TextMate pattern 代替语义判断。 |
...
| CSS 诊断 | 已实现 CSS Language Service validation、宿主 range 映射与 `yak.css.validate` 资源设置；过滤插值、wrapper 和已知插值邻接误报。 | 有语法/规则诊断，并把结果映射回 TS/JS 源文件。 | 后续映射可配置 CSS lint 与 custom data；yak 专属语义规则仍由项目配置的 `eslint-plugin-yak` 负责。 |
...
| 配置 | 当前提供 `yak.css.validate`；tag 名、额外 import module、颜色装饰、Emmet 排序和 CSS data 仍不可配置。 | 可配置 tags、lint、Emmet 选项。 | 中优先级：支持确认过的额外 tag/import module、用户 CSS custom data 与 CSS lint 配置映射。 |
...
| 维护状态 | 本项目可按 yak 的当前版本演进。 | VS Code styled-components 仓库官方说明自 2024-06 起不再由团队维护；TypeScript plugin 默认分支的最近提交也较旧。 | 学习架构和测试策略，不应无审查地复制旧依赖、旧 grammar 或历史 API。 |
...
不建议现在重写为 TypeScript server plugin。独立 VS Code 扩展更容易随 yak API 发布，且不会要求用户切换到工作区 TypeScript 版本。
...
yak 官方文档称其为标准 CSS 语法，并明确支持嵌套、keyframes 与媒体查询。下面的写法应作为本扩展的目标体验：
...
截至本文调研基线，yak 的官方文档与 `css_in_js_parser` 源码没有声明或引入 Sass/SCSS/Less 编译器；解析器明确处理 CSS scope nesting。故下列预处理器特性不应被本扩展宣称为受 yak 支持：
...
因此不应从“竞品用了 SCSS parser”推导出“yak 应支持完整 SCSS”；更不能推导出 Less 支持。

### 未来若要支持预处理器

只有同时满足以下条件时，才应考虑加入可选 SCSS 或 Less 模式：

1. yak 上游明确宣布并测试对应语法的编译支持。
2. 构建产物的 CSS 处理链能可靠处理该语法，而不是只让编辑器看起来不报错。
3. 本扩展针对高亮、补全、hover、诊断、代码操作和混合 `${...}` 插值都有 fixture 与端到端测试。
4. 默认模式仍保持 `css`，避免现有 yak 项目因编辑器误报或错误补全而回归。

在这之前，最合适的演进是增强现代 CSS 和 yak 专属模板识别，而不是引入伪 SCSS/LESS 支持。

## 取证与调研基线

- 当前实现：[`src/extension.ts`](../src/extension.ts)、[`package.json`](../package.json)、[`completion-mvp.md`](completion-mvp.md)。
- yak 官方文档：[Getting started](https://yak.js.org/docs/getting-started)、[Features](https://yak.js.org/docs/features)、[Migration from styled-components](https://yak.js.org/docs/migration-from-styled-components)、[How it works](https://yak.js.org/docs/how-does-it-work)。
- yak 上游调研基线：`2ec61c5d89ad39a2f92fa44cb8ff64b871d31926`（2026-08-21）；其中 `css_in_js_parser` 负责 CSS scope nesting。
