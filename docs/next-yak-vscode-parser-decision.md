# next-yak Parser and Bundle Decision

> Decision date: 2026-08-24
>
> Scope: static next-yak import binding detection and tagged-template location.
...
The cache stores the parsed `SourceFile` and the next-yak tagged templates whose import bindings have already been resolved. It is keyed by document URI and reused only while all of these values match:
...
1. Current correctness depends on resolving the identifier used by a tag back to a specific `next-yak` import symbol. That rejects shadowed function parameters and local bindings.
