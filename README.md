...
This extension owns static CSS language features inside recognized next-yak templates. It does not load workspace ESLint configurations or execute ESLint rules itself. Use the official `eslint-plugin-yak` with the VS Code ESLint extension for next-yak-specific semantic diagnostics and fixes, such as nesting-selector checks, `:global()` migration warnings, semicolon enforcement for template expressions, and runtime style-condition guidance.
